...
### Советы
* Ознакомьтесь с референсной реализацией виджетов в файлах `button.h` и `placeholder.h`.
...
* Если вам очень требуется какое-то неприватное поле, вы можете заглушить соответствующие предупреждения,
  смотри примеры в `placeholder.h`.

### Лирическое отступление
* Реализации методов, скорее всего, было бы правильнее сделать в отдельных `.cpp` файлах — так `.h` проще читать,
  да и скорость компиляции увеличивается.
  Зато становится больше строк кода, которые надо вам написать, а нам проверить.
  А это задание и так объёмное, так что пишем всё в `.h`.
...
для остальных частей в файле `test_utils.h`, чтобы отключить соответствующие автотесты.
...
## Сроки сдачи
Задание выдано 16 ноября 2021 (вторник),
окончательные уточнения и упрощения (при их наличии) ожидаются не позже 17 ноября 2021 (среда).
Ниже в каждом случае указано московское время.

* **Дедлайн сдачи:** 22 ноября 2021 (понедельник), **22**:59.
* Ожидаемый срок проверки: 29 ноября (понедельник).
...
* **Срок исправлений**: 6 декабря 2021 (понедельник), **22**:59.
